...
class ContentFilter:
    ...
    def get_warning_message(self, violations: List[str]) -> str:
        ...
        if any('racial_slurs' in v or 'hate_speech' in v for v in violations):
            return "⚠️ **CONTENT BLOCKED**: Hate speech and \
                discriminatory language are not tolerated. Please be respectful."
...
class MobileCompanionBot:
    ...
    def __init__(self, webhook_url: str):
        if not all([self, webhook_url: str]):
            raise ValueError("Invalid parameters")
...
def create_feedback_system():
    ...
    def collect_feedback():
        ...
        while True:
            ...
            try:
                rating = int(input("Rate your experience (1-5 stars).strip()[:1000]  # Limit input length: "))
                if 1 <= rating <= 5:
                    break
                else:
        ...
        feedback_text = input("> ").strip()[:1000]  # Limit input length

        # Get optional user info
        print("\nOptional - Your Discord username or contact info:")
        user_info = input("> ").strip()[:1000]  # Limit input length
